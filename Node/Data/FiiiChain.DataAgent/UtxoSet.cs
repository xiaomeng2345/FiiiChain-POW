...
namespace FiiiChain.DataAgent
{
    ...
    {
        protected Dictionary<string, List<UtxoMsg>> MainSet;
        public static UtxoSet Instance;

        public UtxoSet()
        {
            this.MainSet = new Dictionary<string, List<UtxoMsg>>();
        }
        ...
        public long GetAllConfirmedBalance()
        {
            ...
            {
                foreach(var utxo in this.MainSet[key])
                {
                    if(utxo.IsConfirmed)
                    {
                        balance += utxo.Amount;
